...
import { Appointment } from '../utils/ParamList';
...
const ConsultationHistoryItem = ({ item, maxNameWidth }: Props) => {
  const theme = useTheme<Theme>();
  const [menuVisible, setMenuVisible] = useState(false);

  const formatDateTime = () => {
    const date = moment(item?.availability?.date);
    const time = item?.timeslots?.[0]?.startTime;
    return `${date.format('DD MMM, YYYY')} | ${time}`;
  };
    ...
    <Box
      ...
      <Box maxWidth={maxNameWidth} minWidth={maxNameWidth}>
        ...
        <Text
          ...
          color="label"
          marginTop="xs">
          {item?.consultant?.title || 'Consultant'}
        </Text>
      </Box>
      <Box
        flex={1}
            ...
            <MenuItem
              ...
              <Box flexDirection="row" alignItems="center" gap="s">
                <MaterialCommunityIcons
                  ...
                  color={theme.colors.label}
                />
                <Text fontSize={16}>Rate Consultant</Text>
              </Box>
            </MenuItem>
            <MenuItem
              onPress={() => {
